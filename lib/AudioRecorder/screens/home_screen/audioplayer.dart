...
class AudioPlayerState extends State<AudioPlayer> {
  ...
  Future<void> stop() async {
    ...
    // Save SOS data after uploading audio
    await _saveSOSData();
  }
  ...
  Future<void> _captureAndSaveImage() async {
    ...
    if (image != null) {
      ...
      final Reference ref =
      FirebaseStorage.instance.ref().child('images').child('image_${DateTime.now()}.jpeg');
      final UploadTask uploadTask = ref.putFile(
        file,
        SettableMetadata(contentType: 'image/jpeg'),
      );
      ...
      // Save SOS data after uploading image
      await _saveSOSData();
    } else {
      Fluttertoast.showToast(msg: "No image captured");
    }
  ...
  Future<void> _saveSOSData() async {
    // Check if variables are null or empty
    // if (_curentPosition == null) {
    //   Fluttertoast.showToast(msg: "Current location is not available");
    //   return;
    // }
    if (imageUrl.isEmpty) {
      Fluttertoast.showToast(msg: "Image URL is not available");
      return;
    }
    if (audioUrl.isEmpty) {
      Fluttertoast.showToast(msg: "Audio URL is not available");
      return;
    }

    final docRef = FirebaseFirestore.instance.collection('sos').doc();
    await docRef.set({
      'lat':'12.8406',
      'long':'80.1534',
      // 'geolocation': 'https://maps.google.com/?daddr=${_curentPosition?.latitude},${_curentPosition?.longitude}',
      'image': imageUrl,
      'audio': audioUrl,
      'timestamp': FieldValue.serverTimestamp(),
    });

    Fluttertoast.showToast(msg: "SOS data saved successfully");
  }
